...
- **[Medium] PMWFeeProof: bind the transaction `Response` JSON to its indexed row before trusting the `Fee`.**
  `computeActualFee` (`internal/attestation/pmwfeeproof/xrp/verifier.go:178`) fetches XRP transactions by `(sourceAddress, nonces)`, then `parseTxFee` (`internal/attestation/pmwfeeproof/xrp/verifier.go:202`) reads only the `Fee` field out of the row's `Response` JSON blob and sums it. It never confirms that the blob actually describes the row it was fetched for. PMWPaymentStatus protects the same trust boundary: `checkRowConsistency` (`internal/attestation/pmwpaymentstatus/xrp/verifier.go:65,80`) cross-checks the JSON's `Hash`/`Account`/`Sequence` against the canonical DB columns before use. FeeProof should do the same per-nonce before `actualFee.Add(...)`. Note: the *row* is already bound to `(sender, nonce)` (the result map is keyed by `tx.Sequence` and looked up by nonce), and `Fee` itself has no column equivalent, so the only available defense is verifying the blob's embedded Account/Sequence/Hash match the columns. This sits inside the documented L-02 trust boundary (partial write / targeted field tamper of an otherwise-trusted indexer; a fully compromised indexer is explicitly out of scope). Suggested fix: lift `checkRowConsistency` into a shared helper (it currently lives in the paymentstatus package) and call it in `computeActualFee`.
