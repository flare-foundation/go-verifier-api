...
9. Build FDC2 response (`builder.BuildPaymentStatusResponse`):
   ...
   - impossible drops magnitudes fail closed: `Amount`, `MaxFee`, `TransactionFee`, and `ReceivedAmount` above the total XRP supply (`helper.MaxXRPDrops` = 1e17 drops) → `ErrDatabase` → 503. (Defense-in-depth on the indexer-derived outcome fields, which are otherwise semi-trusted.)
