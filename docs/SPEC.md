...
8. **Row-consistency check** (`db.CheckRowConsistency`): the identity fields parsed from the response JSON (`hash`, `Account`, `Sequence`) must match the row's indexed columns (`Hash`, `SourceAddress`, `Sequence`) before its payload is used — this binds the row's *identity*, not the transaction *outcome* (see "Indexer payload is semi-trusted" under Accepted risks); a mismatch is treated as a DB inconsistency (`ErrDatabase` → 503). Response rows over `maxResponseSize` are rejected the same way.
9. Build FDC2 response (`builder.BuildPaymentStatusResponse`):
   ...
   - status/revert reason from raw tx result — only validated-ledger result classes are accepted: `tes*` (success) and `tec*` (failed but included in the ledger). Local/pre-consensus classes (`tef`/`tel`/`tem`/`ter`) and unrecognized codes fail closed (`ErrDatabase` → 503) rather than being attested as a real on-ledger outcome. This assumes the source XRP indexer serves **validated-ledger transactions only** (the operational contract for the indexer feeding `SOURCE_DATABASE_URL`)
   ...
   - impossible drops magnitudes fail closed: `Amount`, `MaxFee`, `TransactionFee`, `ReceivedAmount`, and the individual `AccountRoot` balances `ReceivedAmount` is derived from (`final`/`previous`/created-node `Balance`, checked at parse time so an impossible absolute balance can't yield a plausible delta) above the total XRP supply (`helper.MaxXRPDrops` = 1e17 drops) → `ErrDatabase` → 503. (Defense-in-depth on the indexer-derived outcome fields, which are otherwise semi-trusted.) Decimal drops strings (`Fee`, `AccountRoot` balances) are also length-capped before parsing (`helper.ParseNonNegativeBigInt`, `maxDropsDigits`) so an overlong value cannot drive quadratic-time `big.Int` parsing.
...
### Accepted risks
- **Indexer payload is semi-trusted** (`db.CheckRowConsistency` / `db_transaction.go`): the verifier binds each indexed row/event to the request by *identity* — instruction-ID recompute + `CheckInstructionConsistency`, the `hash`/`Account`/`Sequence` row check, and the on-chain `initialNonce` sequence binding — and fails closed on duplicate/missing rows and oversize responses. It then reads *payload content* from the indexer JSON that it cannot independently verify against the source ledger: for PMWPaymentStatus the transaction **outcome** (`TransactionStatus`/`RevertReason` from `metaData.TransactionResult`, `ReceivedAmount` from `AffectedNodes`) and `TransactionFee`; for PMWFeeProof the per-payment `maxFee`/`Fee`. A fully compromised indexer could misreport these for a correctly-identified payment.
  - *Compensating controls*: (1) FDC2 attestations require agreement across multiple independent verifiers, each with its own indexer, so a single tampered indexer is outvoted rather than decisive; (2) the identity bindings above prevent substituting a *different* payment or transaction; (3) physically-impossible values fail closed — XRP amounts/fees above the total XRP supply (`helper.MaxXRPDrops`) for both PMW types, `estimatedFee`/`actualFee` sums exceeding `uint256` (PMWFeeProof), and result codes outside the validated-ledger classes `tes`/`tec` (PMWPaymentStatus).
  - *Residual risk*: a compromised indexer can still corrupt the reported outcome/fee of a correctly-identified payment within physically-plausible bounds; this is mitigated by multi-verifier consensus, not by single-verifier verification.
