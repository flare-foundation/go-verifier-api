...
- **HTTP redirects not re-validated** (`fetcher.go`): The HTTP client follows redirects (Go default, up to 10 hops). The initial URL is validated against SSRF controls (private/dangerous IP blocking, DNS pinning), but redirect targets are not re-validated. This is accepted because: (1) some TEE proxy setups (ngrok, cloudflare tunnels, HTTP→HTTPS upgrades) rely on redirects; (2) exploiting this requires a malicious TEE operator who could already serve arbitrary data; (3) the service is deployed in internal infrastructure with network-level egress controls. Compensating controls: DNS pinning on initial URL, 2MB response size limit, request timeouts.
- **Unbounded ABI event data decoding** (`instruction_event.go`): `DecodeTeeInstructionsSentEventData` decodes `log.Data` without an explicit size cap. The event data size is bounded in practice by: (1) C-chain block gas limits constrain the maximum emitted event size; (2) the c-chain indexer only stores logs from configured contract addresses; (3) emitting a large event costs significant gas. A future hardening step could add an explicit `len(log.Data)` check before ABI decoding.
