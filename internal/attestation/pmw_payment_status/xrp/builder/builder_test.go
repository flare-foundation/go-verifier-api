...
func TestBuildPaymentStatusResponse(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		require.Equal(t, val.Amount, paymentMessageInstruction.Amount)
		require.Equal(t, val.BlockNumber, txFromDB.BlockNumber)
		require.Equal(t, val.BlockTimestamp, txFromDB.Timestamp)
		require.Equal(t, val.TokenId, paymentMessageInstruction.TokenId)
		require.Equal(t, val.RecipientAddress, paymentMessageInstruction.RecipientAddress)
		require.Equal(t, val.PaymentReference, paymentMessageInstruction.PaymentReference)
		require.Equal(t, val.RevertReason, "")
		require.Equal(t, val.ReceivedAmount, paymentMessageInstruction.Amount)
		require.Equal(t, val.TransactionFee, paymentMessageInstruction.Fee)
		require.Equal(t, val.TransactionStatus, uint8(types.Success))
		require.Equal(t, hex.EncodeToString(val.TransactionId[:]), strings.ToLower(txFromDB.Hash))
	})
	t.Run("success - different status", func(t *testing.T) {
		...
		require.Equal(t, val.RecipientAddress, paymentMessageInstruction.RecipientAddress)
		require.Equal(t, val.PaymentReference, paymentMessageInstruction.PaymentReference)
		require.Equal(t, val.RevertReason, "tecNO_DST_INSUF_XRP")
		require.Equal(t, val.ReceivedAmount, paymentMessageInstruction.Amount)
		require.Equal(t, val.TransactionFee, paymentMessageInstruction.Fee)
		require.Equal(t, val.TransactionStatus, uint8(types.ReceiverFault))
		require.Equal(t, hex.EncodeToString(val.TransactionId[:]), strings.ToLower(txFromDB.Hash))
	})
