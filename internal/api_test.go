...
func TestPMWMultisig(t *testing.T) {
	...
	resp, err := testutil.Post(t, "http://localhost:3120/verifier/xrp/PMWMultisigAccountConfigured/verify", types.AttestationRequest{
...
func TestPMWPaymentStatus(t *testing.T) {
	...
	t.Run("Test valid payment", func(t *testing.T) {
		...
		resp, err := testutil.Post(t, "http://localhost:3121/verifier/xrp/PMWPaymentStatus/verify", types.AttestationRequest{
			AttestationType: attestationType,
			SourceID:        sourceId,
			RequestBody:     attestationRequest,
		}, testAPIKey)
	...
	t.Run("Test invalid sourceId", func(t *testing.T) {
		...
		_, err = testutil.Post(t, "http://localhost:3121/verifier/xrp/PMWPaymentStatus/verify", types.AttestationRequest{
			AttestationType: attestationType,
	...
	t.Run("Test prepareRequestBody", func(t *testing.T) {
		...
		resp, err := testutil.Post(t, "http://localhost:3121/verifier/xrp/PMWPaymentStatus/prepareRequestBody", request, testAPIKey)
		require.NoError(t, err)

		var response types.AttestationRequestEncoded
		require.NoError(t, json.Unmarshal(resp, &response))
	...
	t.Run("Test prepareRequestBody", func(t *testing.T) {
		...
		ftdReq := types.AttestationRequest{
			AttestationType: common.BytesToHash(attestationType[:]),
			SourceID:        common.BytesToHash(sourceId[:]),
			RequestBody:     attestationRequest,
		}
		...
		var response types.AttestationResponseData[types.PMWPaymentStatusResponseBody]
		require.NoError(t, json.Unmarshal(resp, &response))
